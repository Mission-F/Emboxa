...
function renderFolders(){
  $('#folder-list').innerHTML=`<button class="folder-item ${!state.trash&&state.folderId===null?'active':''}" data-folder="">${icon('mail')}<b>${t('allMessages')}</b><em>${numberFmt(state.account.message_count)}</em></button>`+state.folders.map(folder=>{const short=folder.remote_count!=null&&folder.message_count<folder.remote_count;return `<button class="folder-item depth-${folderDepth(folder.name)} ${!state.trash&&state.folderId===folder.id?'active':''} ${short?'folder-short':''}" data-folder="${folder.id}" title="${esc(folder.name)}${short?` — ${t('missingWarning')}: ${numberFmt(folder.message_count)} / ${numberFmt(folder.remote_count)}`:''}">${icon(folderIcon(folder))}<b>${esc(folderLeaf(folder.name))}</b><em>${short?`${numberFmt(folder.message_count)}/${numberFmt(folder.remote_count)}`:numberFmt(folder.message_count)}</em></button>`;}).join('')+`<button class="folder-item ${state.trash?'active':''}" data-folder="trash">${icon('trash')}<b>${t('trashFolder')}</b><em>${numberFmt(state.deletedCount)}</em></button>`;
}
...
async function loadStats(){try{const stats=await api(`/api/accounts/${state.account.id}/stats?${snapshotParam()}`);state.deletedCount=stats.deleted||0;const span=stats.oldest&&stats.newest?`<span class="archive-range">${t('coverageFrom')} <b>${dateOnly(stats.oldest)}</b> ${t('coverageTo')} <b>${dateOnly(stats.newest)}</b></span>`:'';
