...
# Nor did the patient waits on a single message ever pay off: three full Yahoo runs, a dozen
# retry sequences, not one recovery — and every refusal costs about six seconds before the
...
PUBLIC_APP_URL = os.getenv("PUBLIC_APP_URL", "https://app.emboxa.eu").rstrip("/")
...
STANDARD_STORAGE_LIMIT_BYTES = int(os.getenv("STANDARD_STORAGE_LIMIT_BYTES", str(5 * 1024**3)))
STANDARD_MAILBOX_LIMIT = int(os.getenv("STANDARD_MAILBOX_LIMIT", "2"))
